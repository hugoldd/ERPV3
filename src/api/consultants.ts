...
export async function fetchConsultants(): Promise<Consultant[]> {
  if (!HAS_ORGANIZATION) return []; // ✅ empêche la requête
...
export async function createConsultant(input: Omit<Consultant, "id">): Promise<void> {
  if (!HAS_ORGANIZATION) throw new Error("Organisation non configurée (VITE_ORGANIZATION_ID).");
  ...
  if (competenceIds.length) {
    const { error: e2 } = await supabase.from("consultant_competences").insert(
      competenceIds.map((cid) => ({ consultant_id: data.id, competence_id: cid }))
    );
    if (e2) throw e2;
  }
}
